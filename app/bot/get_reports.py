...
async def get_report_from_db(session: AsyncSession, from_date=None, to_date=None):
    """
    ...
    messages = await crud_message.get_by_date_interval(from_date, to_date, session)
    users_senders = []
    answers_amount = 0
    questions_amount = 0

    for mes in messages:
        if mes.telegram_user_id not in users_senders and not mes.answer_to_user_id:
