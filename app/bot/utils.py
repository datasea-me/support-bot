...
from app.schemas.user import UserBaseScheme, UserFromDBScheme
...
async def get_telegram_user_from_resend_message(message: Message, bot: Bot):
    ...
    if not telegram_user_id:
        return await message.reply(text='Невозможно найти пользователя с таким Id')
    try:
        return await bot.get_chat(telegram_user_id)
    except TelegramAPIError as err:
        return await message.reply(text=(f'Невозможно найти пользователя с таким Id. Текст ошибки:\n{err.message}'))
